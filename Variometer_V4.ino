...
float noiseConstant = 0.1;
float tempo = 0.0;

// for reading non important sensors for every second 
unsigned long get_time_second = millis();

static const float noiseD1 = 0.1;
static const float noiseD2 = 0.05;
static const float noiseD3 = 0.01;
...
{    
    ...
    if(screen == 2)
    {
        ...
        if(m_clock == 1)
        {          
             if(countPressVal == 2 || countPressVal == 3 || countPressVal == 4)
            {
                u8g.drawStr(0, 30, "D");                  
                u8g.setPrintPos(10, 30);
                int noiseTmp;
                if((countPressVal) == 2) {noiseTmp = 1;}    // 3 sinal  Vario damper 1 more sensitive
                if((countPressVal) == 3) {noiseTmp = 2;}   // 4 sinal  Vario damper 2
                if((countPressVal) == 4) {noiseTmp = 3;}  // 5 sinal  Vario damper 3
            ...
            if(noiseConstant == noiseD1)   u8g.print("D1");  ; // 3 sinal Vario damper 1 more sensitive
            if(noiseConstant == noiseD2)   u8g.print("D2");  ; // 4 sinal Vario damper 2
            if(noiseConstant == noiseD3)   u8g.print("D3");  ; // 5 sinal Vario damper 3               
...
void setnmeaVarioLXWP0(float alt, float vario) { //in m/s
  ...
  //
  // 0 loger_stored : [Y|N] (not used)
  // 1 IAS [km/h] ----> Condor uses TAS!
  // 2 baroaltitude [m]
  // 3-8 vario values [m/s] (last 6 measurements in last second)
  // 9 heading of plane (not used)
  // 10 windcourse [deg] (not used)
  // 11 windspeed [km/h] (not used)
  //
  // e.g.:
  // $LXWP0,N,,665.55,1.71,,,,,,,,*E4
...
{   
    ...
    else if(time >5 && time <= 10)//Lets fly screen
    {
        screen = 1;
    }
    else
    ...
    {   
        if ((millis() - duration > 5) && (millis() - duration < 2000) && menu == 2 && buttonStatus == 0)
        {
            ...
            if(countPressVal > 5){countPressVal = 0;}
    ...
    if (lastMillis > 0 && (millis() - lastMillis > 2500))
    {
        ...
        if(menu == 2)
        {
            ...
            if((countPressVal) == 2) 
            {
              noiseConstant = noiseD1; 
              varioKalmanFilter.setProcessNoise(noiseConstant);
              //altitudeKalmanFilter.setProcessNoise(noiseConstant);
            }    // 3 sinal Vario damper 1 more sensitive
            if((countPressVal) == 3) 
            {
              noiseConstant = noiseD2; 
              varioKalmanFilter.setProcessNoise(noiseConstant);
              //altitudeKalmanFilter.setProcessNoise(noiseConstant);
            }   // 4 sinal Vario damper 2
            if((countPressVal) == 4) 
            {
              noiseConstant = noiseD3; 
              varioKalmanFilter.setProcessNoise(noiseConstant);
              //altitudeKalmanFilter.setProcessNoise(noiseConstant);
            }  // 5 sinal Vario damper 3
            if((countPressVal) == 5) 
            {
                if(beepOn)
                    beepOn = false;
    ...
    //every 0.2 second get temperature and send LXWP0 nmea
    if (millis() >= (get_time_second + 200))      
    {
